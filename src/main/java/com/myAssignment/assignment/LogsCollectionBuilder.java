...
public class LogsCollectionBuilder implements Parsable<File> {
	...
	public List<ServerLog> parseFile(File f) {
		...
		try (Scanner input = new Scanner(file).useDelimiter(pattern)) {
			while (input.hasNext()) {
				...
				if (line != "") {
					...
					logList.add(new ServerLog(obj.getString("id"), obj.getString("state"), obj.getLong("timestamp"),
							hostName, typeOfMessage));
				} else {
					break;
				}
			}
		...
		sb.append("The .json file in the path: (").append(this.file).append(") has been successfully scanned.\n")
				.append("All of the logs (").append(logList.size()).append(") are collected in a list.");
